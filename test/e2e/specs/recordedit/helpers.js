            ...
            it("should show columns with generated or immutable annotations as disabled", function() {
                ...
                for (var annotationKey in disabledCols) {
                    disabledCols[annotationKey].forEach(function(column) {
                        if (column.type.typename === 'timestamp' || 'timestamptz') {
                            ...
                            expect(dateInput.isEnabled()).toBeFalse();
                            expect(timeInput.isEnabled()).toBeFalse();
                            expect(meridiemInput.isEnabled()).toBeFalse();
                        } else {
                            chaisePage.recordEditPage.getInputForAColumn(column.name, recordIndex).then(function(input) {
                                ...
                                if (annotationKey === GENERATED) {
                                    expect(i.getAttribute('placeholder')).toBe('Automatically generated by the server');
                                }
                            }).catch(function(e) {
                                console.log(e);
                            });
