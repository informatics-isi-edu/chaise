...
it("should show " + tableParams.columns.filter(function(c) {
  return c.value != null;
}).length + " columns only", function() {
...
it("clicking the related table heading should change the heading and hide the table.", function() {
  ...
  tableHeading.element(by.css('.glyphicon')).getAttribute('class').then(function(cssClass) {
    ...
    browser.wait(EC.elementToBeClickable($(".accordion-toggle")), browser.params.defaultTimeout);

    // browser.wait(EC.elementToBeClickable(element(by.css(".accordion-toggle")), browser.params.defaultTimeout));
