...
request = request.defaults({
  headers: {'cookie': 'ermrest=C6KFIQn2JS37CGovofWnjKfu;'}
});

exportEntities = function(table) {
	request
	  .get('https://dev.isrd.isi.edu/ermrest/catalog/283/entity/product:' + table)
	  .pipe(fs.createWriteStream('./data/product/' + table + '.json'));
};
