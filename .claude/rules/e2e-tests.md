...
| Group | Purpose | Notes |
|---|---|---|
| `all-features` | Full feature tests, some mutate the catalog model | Those specs hold the `TEST_LOCKS.CATALOG_MODEL` lock |
...
hold them, so other specs must not assert on the state being changed.
