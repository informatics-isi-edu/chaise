...
// utilties
import {
  chaiseURItoErmrestURI,
  createRedirectLinkFromPath,
} from '@isrd-isi-edu/chaise/src/utils/uri-utils';
...
import {
  addAppContainerClasses,
  updateHeadTitle,
} from '@isrd-isi-edu/chaise/src/utils/head-injector';
...
const ERDApp = (): JSX.Element => {
  ...
  //   if (res.queryParams && 'savedQueryRid' in res.queryParams)
  //     logObject.sq_rid = res.queryParams.savedQueryRid;
  //   if (res.isQueryParameter) logObject.cqp = 1;

  //   ConfigService.ERMrest.resolve(res.ermrestUri)
  //     .then((response: any) => {
  ...
  //         addAlert(
  //           MESSAGE_MAP.previousSession.message,
  //           ChaiseAlertType.WARNING,
  //           AuthnStorageService.createPromptExpirationToken,
  //           true
  //         );
  //       }
  ...
  //       const logStack = [
  //         LogService.getStackNode(LogStackTypes.SET, reference.table, reference.filterLogInfo),
  //       ];
