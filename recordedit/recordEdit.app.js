                ...
                if (context.mode == context.modes.EDIT || context.mode == context.modes.COPY) {
                    ...
                    if ($rootScope.reference.canCreate) {
                        ...
                        angular.forEach($rootScope.reference.columns, function(column) {
                            ...
                            // checking for not null is required as well so timestamps are initialized when they should be disabled
                            // check the recordEditModel to make sure a value wasn't already set based on the prefill condition
                            if (column.default !== undefined && typeof column.default !== "function" && column.default !== null && !recordEditModel.rows[0][column.name]) {
                            ...
                            // we don't want to populate the model with empty objects that are just going to get set to null
                                ...
                                // all other column types
                                    ...
                                    recordEditModel.rows[0][column.name] = null;
                                }
                            }
                        });
