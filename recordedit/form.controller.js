    ...
    .controller('FormController', ['AlertsService', 'ConfigUtils', 'dataFormats', 'DataUtils', 'ErrorService', 'InputUtils', 'integerLimits', 'logService', 'maskOptions', 'messageMap', 'modalBox', 'modalUtils', 'recordCreate', 'recordEditAppUtils', 'recordEditModel', 'recordsetDisplayModes', 'Session', 'UiUtils', 'UriUtils', '$cookies', '$document', '$log', '$rootScope', '$scope', '$timeout', '$window',
        ...
        function onSuccess (model, result){
            ...
            if (model.rows.length == 1) {
                ...
                // so we modify our own object instead of one returned from ermrestJS APIs
                var resultsTuples = page.tuples,
                    failedTuples = [];

                // NOTE: record-table directive uses vm.columnModels (generated by the directive using vm.reference) and vm.rowValues (supplied by us)
                // to remove pseudo columns in this display which won't get data to display, remove values from tuples and let directive remove columns
                // go backwards through the set so as we remove values, the later values in the array don't shift forward in index before we see them
                for (var i=resultsReference.columns.length-1; i>=0; i--) {
                    var col = resultsReference.columns[i];

                    // if we want to remove this column from display
                    // we want to return all values that have a differing index than the one we are on
                    if (col.isPseudo && col.isPathColumn) {
                        // 1 tuple at a time, remove the value and corresponding entry in isHTML
                        resultsTuples.forEach(function (tuple) {
                            tuple.values.splice(i, 1);
                            tuple.isHTML.splice(i, 1);
                        });

                        // same logic as above but for failed tuples for inconsistency
                        if (failedPage !== null) {
                            // so we modify our own object instead of one returned from ermrestJS APIs
                            failedTuples = failedPage.tuples;

                            failedTuples.forEach(function (tuple) {
                                tuple.values.splice(i, 1);
                                tuple.isHTML.splice(i, 1);
                            });
                        }
