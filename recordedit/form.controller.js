(function() {
  ...
  angular.module('chaise.recordEdit')
    ...
    .controller('FormController', ['AlertsService', 'DataUtils', 'ErrorService', 'recordEditModel', 'UriUtils', '$cookies', '$log', '$rootScope', '$timeout', '$uibModal', '$window', 'Session', 'messageMap', '$http', function FormController(AlertsService, DataUtils, ErrorService, recordEditModel, UriUtils, $cookies, $log, $rootScope, $timeout, $uibModal, $window, Session, messageMap, $http) {
      var vm = this;
      ...
      //vm.markdownPreview = markdownPreview;      //Issue#794
      ...
      function areFilesValid(rows) {
        ...
        rows.forEach(function(row) {
          ...
          for (var k in row) {
            ...
            try {
              var column = $rootScope.reference.columns.find(function(c) {
                return c.name == k;
              });
              if (column.isAsset) {
                ...
                if (row[k].url == "" && !column.nullok) {
                  ...
                  AlertsService.addAlert({
                    type: 'error',
                    message: "Please select file for column " + k + " for record " + index
                  });
                  ...
                  try {
                    if (!row[k].hatracObj.validateURL(row)) {
                      ...
                      AlertsService.addAlert({
                        type: 'error',
                        message: "Invalid url template for column " + k + " for record " + index
                      });
                    }
                  } catch (e) {
                    isValid = false;
      ...
      function addRecords(isUpdate) {
        ...
        uploadFiles(submissionRowsCopy, isUpdate, function() {
          ...
          var fn = "create",
            fnScope = $rootScope.reference.unfilteredReference.contextualize.entryCreate,
          ...
          fnScope[fn].apply(fnScope, args).then(function success(page) {
            ...
            if (model.rows.length == 1) {
              ...
              AlertsService.addAlert({
                type: 'success',
                message: 'Your data has been submitted. Showing you the result set...'
              });
      ...
      function toggleMeridiem(modelIndex, columnName) {
        ...
        if (!vm.recordEditModel.rows[modelIndex][columnName]) {
          vm.recordEditModel.rows[modelIndex][columnName] = {
            meridiem: 'AM'
          };
        }
        // Do the toggling
